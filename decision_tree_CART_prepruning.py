...
def prePrune(dataSet,bestFeat,bestFeatLabel,bestFeatValue):
    majority = majorityClass(dataSet)
    counts = classCount(dataSet)
    correct = counts[majority]
    beforeProb = float(correct) / len(dataSet)
    subCorrect = 0
    if type(dataSet.iloc[0,bestFeat]).__name__=='float64':
        for i in range(2):
            subDataSet=splitContinuousDataSet(dataSet,bestFeat,bestFeatValue,i)
            subMajority=majorityClass(subDataSet)
            subCounts=classCount(subDataSet)
            subCorrect+=subCounts[subMajority]
        afterProb=float(subCorrect)/len(dataSet)
    if type(dataSet.iloc[0,bestFeat]).__name__=='str':
        uniqueVals=set(dataSet.iloc[:,bestFeat])
        for value in uniqueVals:
            ...
            subCorrect+=subCounts[subMajority]
        afterProb=float(subCorrect)/len(dataSet)
    print('划分前：'+str(beforeProb))
    print('划分后：'+str(afterProb))
    return afterProb>=beforeProb
...
def createTree(dataSet):
    ...
    # pre prune
    if prePrune(dataSet,bestFeat,bestFeatLabel,bestFeatValue):
        ...
        if type(dataSet.iloc[0, bestFeat]).__name__ == 'float64':
            greaterDataSet = splitContinuousDataSet(dataSet, bestFeat, bestFeatValue, 0)
            createTree(greaterDataSet)
            smallerDataSet = splitContinuousDataSet(dataSet, bestFeat, bestFeatValue, 1)
            createTree(smallerDataSet)
        if type(dataSet.iloc[0, bestFeat]).__name__ == 'str':
            bestFeatValues = dataSet.iloc[:, bestFeat]
            uniqBestFeatVals = set(bestFeatValues)
            for value in uniqBestFeatVals:
